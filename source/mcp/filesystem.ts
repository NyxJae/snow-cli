...
export const mcpTools = [
	...
	},
	{
		...
		description:
			'Create a new file with content. **PATH REQUIREMENT**: Use EXACT non-empty string path, never undefined/null/empty/placeholders like "path/to/file". Verify file does not exist first. Automatically creates parent directories.注意!为安全起见,如果要创建的文件已存在,则会创建失败,故你需要先使用搜索工具确认文件存在性和内容,避免误覆盖.',
		inputSchema: {
			type: 'object',
			properties: {
				filePath: {
