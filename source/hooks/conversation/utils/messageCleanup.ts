...
/**
 * 动态精简 ace-text_search 工具的返回内容
 *
 * 精简规则:
 * 1. 只精简 ace-text_search 工具的返回内容
 * 2. 保留最近的 5 次搜索结果
 * 3. 5次之前且超过 10 分钟的搜索结果才精简
 * 4. 保留所有错误结果
 * 5. 将旧搜索结果替换为"本次搜索结果已过时,故精简为占位符"
 *
 * 重要: 此函数修改传入的消息数组,调用方应确保传入的是副本而非原始数据
 * 这样可以确保精简操作只影响发送给API的消息,不修改存储的历史消息
 *
 * @param messages - 消息数组(将被就地修改)
 */
export function simplifyTextSearchResults(messages: ChatMessage[]): void {
	const now = Date.now();
	const TIMEOUT_THRESHOLD = 10 * 60 * 1000; // 10分钟(毫秒)
	const KEEP_RECENT_COUNT = 5; // 保留最近5次搜索

	// ========================================
	// 步骤1: 构建工具调用ID到工具名称的映射
	// ========================================
	...
	// ========================================
	// 步骤2: 收集所有 text_search 工具返回消息
	// ========================================
	interface TextSearchResultInfo {
		index: number;
		timestamp: number;
		isError: boolean;
	}

	const textSearchResults: TextSearchResultInfo[] = [];

	for (let i = 0; i < messages.length; i++) {
		const msg = messages[i];
		if (!msg) continue;

		// 只处理有timestamp的tool消息,且是text_search工具
		if (
			msg.role === 'tool' &&
	...
	// ========================================
	// 步骤3: 按timestamp降序排序(最新的在前)
	// ========================================
	textSearchResults.sort((a, b) => b.timestamp - a.timestamp);
	...
	for (let i = 0; i < textSearchResults.length; i++) {
		const result = textSearchResults[i];
		if (!result) continue;

		// 保留最近5次搜索(已按时间戳排序,前5个就是最新的)
		if (i < KEEP_RECENT_COUNT) {
			continue;
		}
	...
	if (indicesToSimplify.length > 0) {
		console.log(
			`[simplifyTextSearchResults] Simplified ${indicesToSimplify.length} text search results`,
		);
	}
}
