 ...
 *
 * 安全分析主代理
 * 特点：专注漏洞发现与验证，完全采用上游 Vulnerability Hunting Mode 设计
 */

import type {MainAgentConfig} from '../../types/MainAgentConfig.js';
import {
	DEFAULT_TOOL_PERMISSIONS_FOR_TEAM, // 使用Team的权限作为基础
	DEFAULT_SUB_AGENTS_FOR_TEAM, // 使用Team的子代理作为基础
	BUILTIN_MAIN_AGENTS,
 ...
 * 获取debugger主代理配置
 * 
 * 完全采用上游 Vulnerability Hunting Mode 的设计：
 * - 系统提示词：使用上游 vulnerabilityHuntingModeSystemPrompt.ts 的内容
 * - 功能定位：代码漏洞分析、安全审计
 * - 核心能力：
 *   - 逻辑 Bug 和代码质量问题分析
 *   - 业务逻辑缺陷检测
...
export function getSnowDebuggerConfig(): MainAgentConfig {
	return {
		basicInfo: {
			...
			name: 'Debugger',
			description: '安全分析主代理，专注漏洞发现与验证，融合上游 Vulnerability Hunting Mode',
			type: 'debugger',
			builtin: true,
			createdAt: new Date().toISOString(),
			updatedAt: new Date().toISOString(),
		},
		tools: DEFAULT_TOOL_PERMISSIONS_FOR_TEAM, // 使用Team权限，确保有足够工具进行安全分析
		availableSubAgents: DEFAULT_SUB_AGENTS_FOR_TEAM, // 使用Team子代理，支持安全分析任务委派
		systemPrompt: `你是 Snow AI CLI - Vulnerability Hunting Mode，一个专门的安全分析代理，专注于发现和验证代码库中的安全漏洞。
