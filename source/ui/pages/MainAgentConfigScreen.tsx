...
type Props = {
	...
	onSave: () => void;
	inlineMode?: boolean;
	agentId: string; // 主代理ID（general 或 team）
};
...
export default function MainAgentConfigScreen({
	...
	const handleSave = useCallback(() => {
		...
		try {
			// 检查是否为重置操作（与内置配置完全相同）
			const builtinConfigs = getBuiltinMainAgentConfigs();
			const builtinAgent = builtinConfigs[agentId];
			...
			if (isResetToBuiltin) {
				// 重置操作：删除对应的主代理配置
				if (configFile.agents[agentId]) {
					delete configFile.agents[agentId];
				...
				// 正常保存操作：创建或更新配置
				const existingAgent = configFile.agents[agentId];

				if (!existingAgent && !builtinAgent) {
					setSaveError(`Agent ${agentId} not found`);
					return;
				}
				...
				const baseAgent = existingAgent || builtinAgent!;
				...
				configFile.agents[agentId] = updatedAgent;
				saveMainAgentConfig(configFile);
			}
