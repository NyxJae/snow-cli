...
export default function ChatScreen({autoResume, enableYolo}: Props) {
	...
	useEffect(() => {
		const syncMainAgentState = () => {
			try {
				const newAgentName = mainAgentManager.getCurrentAgentName();
	...
	function handleSwitchMainAgent() {
		panelState.handleSwitchMainAgent({
			isStreaming: streamingState.isStreaming,
			hasPendingRollback: !!snapshotState.pendingRollback,
			hasPendingToolConfirmation: !!pendingToolConfirmation,
			hasPendingUserQuestion: !!pendingUserQuestion,
	...
	const handleMainAgentSelect = panelState.handleMainAgentSelect;
		...
		<Box flexDirection="column" height="100%" width={terminalWidth}>
			...
			{shouldShowFooter && (
				<ChatFooter
					...
					showMainAgentPicker={panelState.showMainAgentPanel}
					setShowMainAgentPicker={panelState.setShowMainAgentPanel}
					mainAgentSelectedIndex={panelState.mainAgentSelectedIndex}
					setMainAgentSelectedIndex={panelState.setMainAgentSelectedIndex}
