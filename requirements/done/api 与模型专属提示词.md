...
- [x] 在 API 与模型设置中新增字段,名称为 模型专属提示词,默认空字符串.
- [x] 该字段仅作用于当前 API 与模型配置(Profile),不影响其他 Profile.
- [x] 内容类型为纯文本指令,不允许变量或占位符.
- [x] 当字段为空或仅含空白时,视为未配置,不插入任何内容.
- [x] 主代理与子代理的拼接顺序保持一致,插入位置固定在 角色定义 之后,AGENTS.md 之前.
- [x] 完整拼接顺序(两类代理一致): 角色定义 -> 模型专属提示词 -> AGENTS.md -> 系统环境 -> 平台指导 -> 任务完成标识 -> 子代理任务提示词(仅子代理存在).
