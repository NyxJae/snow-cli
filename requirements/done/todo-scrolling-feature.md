...
- [x] 使用连接符显示父子关系：├─、└─、│
- [x] 根据 parentId 构建树形结构
- [ ] 滚动单位是单行 TODO，可能拆分父子关系
- [ ] 自动滚动只看状态，不考虑父子关系

**实现状态备注** (2026-02-06):
- ✅ 已实现: 树状连接符显示 - TodoTree.tsx:42-75 buildTodoTree() 构建树状结构,95-103 生成连接符(├─,└─,│)
- ❌ 未实现: Alt+U/D 滚动功能 - useKeyboardInput.ts 虽有参数定义但无实际处理逻辑
- ❌ 未实现: 7秒自动滚动 - 未找到 setTimeout 7000ms 定时器实现
- ❌ 未实现: 标题栏快捷键指引 - TodoTree 组件未显示 "alt+u/d ⬆/⬇" 提示
