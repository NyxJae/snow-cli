...
- [x] 新增/保留: main-agents.toml 中的 Architect 必须作为"内置主代理"参与上述顺序.
- [x] 删除: 任何 main-agents.toml 未出现的内置主代理必须从代码硬编码中移除(例如 leader, vulnerability_hunter).
- [ ] 子代理列表以 sub-agents.toml 为唯一来源,内置子代理列表与其完全一致.
- [x] 删除: sub-agents.toml 未出现的内置子代理必须从代码硬编码中移除(例如 agent_plan, agent_code_reviewer).
- [x] 新增/保留: sub-agents.toml 中存在的子代理必须作为内置子代理存在(例如 agent_reviewer, agent_explore, agent_general, agent_todo_progress_useful_info_admin).

**实现状态备注** (2026-02-06):

- ✅ 已实现: 删除旧主代理硬编码(leader, vulnerability_hunter) - MainAgentManager.ts:27-32 BUILTIN_AGENT_ORDER 不包含这些代理
- ✅ 已实现: Architect 已加入内置主代理
- ✅ 已实现: 删除旧子代理硬编码(agent_plan, agent_code_reviewer) - subAgentConfig.ts:58-195 BUILTIN_AGENTS 不包含这些
- ⚠️ 部分实现: 主代理/子代理列表仍依赖代码硬编码的 BUILTIN_AGENT_ORDER/BUILTIN_AGENTS 数组,未完全从配置文件读取唯一来源
  - MainAgentManager.ts:88-93 switchToNextAgent() 使用硬编码 builtinOrder 数组
  - subAgentConfig.ts:297-332 getSubAgents() 合并内置代理和用户配置,不是"唯一来源"
