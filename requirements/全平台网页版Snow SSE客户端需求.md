...
9. 日志弹窗仅展示当前会话 SSE 事件流.
10. 未知 SSE `event.type` 必须容错忽略,页面不崩溃.
11. 工具调用与工具结果在聊天主区采用"单卡片状态流转"展示:
...
如果代理回复中有思考内容 则在会话历史区的显示代理回复块中 有个折叠区域 显示 自考内容 且默认折叠

有个bug 切换 配置时 会发送 /profile xxx 从用户信息给AI 切换配置 不应该 发送信息给AI的 只是 服务端切换配置而已.现在造成AI误回复!! 因该是切换api和模型配置的逻辑有问题
3