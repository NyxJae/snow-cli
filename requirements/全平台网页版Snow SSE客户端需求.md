...
- [ ] 正式 Web 客户端保留"压缩"按钮,用于当前会话标准压缩操作.
...
9. 当客户端与 SSE 服务端处于已连接状态时,该服务端下所有正在运行的会话必须自动保持连接,无需用户逐个打开会话.
10. 当客户端从断开状态恢复连接后,必须自动补连该服务端下当前仍在运行的全部会话,以继续实时接收消息.
...
4. 聊天记录区自动置底规则:
   - 默认自动置底到最新消息.
   - 当用户手动滚动到非底部时,暂停自动置底.
   - 当用户手动滚回底部,或用户发送新消息后,恢复自动置底.
5. 聊天区刷新与焦点保持规则:
   - 当页面因收到新消息或状态更新触发刷新时,不得抢夺消息输入框焦点.
   - 若用户正在输入消息,输入内容与光标位置必须保持不变.
   - 焦点仅在用户主动点击输入框或执行显式交互时改变.
6. 提供会话控制按钮:
   - 提供"中断"按钮用于中断当前会话执行.
   - 提供"回退"按钮用于回退到记录点.
   - 回退后,将回退点对应消息原文放入输入框,便于继续编辑和重发.
7. 用户在主代理工作中仍可发送消息,新消息进入队列,并支持撤回后编辑.
8. 聊天视图下 TODO 区域支持滚动,Git 视图下 TODO 区隐藏.
9. 提供日志按钮,点击后打开日志弹窗,点击弹窗右上角 `X` 可关闭.
10. 日志弹窗仅展示当前会话 SSE 事件流.
11. 未知 SSE `event.type` 必须容错忽略,页面不崩溃.
12. 工具调用与工具结果在聊天主区采用"单卡片状态流转"展示:

- 工具调用出现时,立即创建工具调用卡片,展示工具名,简要输入参数,状态为"执行中".
- 工具返回后,更新同一张卡片状态为"成功"或"失败",不在聊天主区额外新增独立结果卡片.
- 卡片支持"查看详情"折叠区,可查看完整输入参数与完整返回内容.
- 若返回内容过长,默认展示摘要,完整内容仅在展开后查看.

12. 若代理回复中包含思考内容:

- 在实时收消息阶段和会话历史回看阶段,都必须在对应代理回复块中提供"思考内容"折叠区.
- 折叠区默认收起,用户可手动展开查看.
- 同一条回复在实时展示与历史回看中的思考块可见性规则必须一致,不得出现"实时不显示,历史重进后才显示"的不一致.
...
1. 子代理不再使用独立专门展示区域,统一并入聊天主区的工具卡片体系.
2. 当触发子代理调用时,创建 1 张子代理工具卡片,并显示:
   - 输入任务摘要(即输入参数摘要).
   - 输入任务详情(默认折叠).
3. 子代理工具卡片状态流转规则:
   - 子代理执行中时,卡片状态为"执行中".
   - 子代理输出最终回复后,同一卡片状态更新为"完成".
4. 子代理卡片内的附加区域:
   - 工作过程区域: 用于实时追加子代理执行过程(如工具调用),默认折叠.
   - 子代理回复区域: 展示子代理最终完整回复,默认折叠.注意不要有省略,服务端完整发送,客户端也完整显示.
5. 条件显示规则:
   - 若未收到工作过程,则不显示"工作过程"区域.
   - 若未收到最终回复,则不显示"子代理回复"区域.
   - 无附加区域时,仅显示输入任务相关信息.
6. 视觉风格需与其他工具调用卡片保持一致,仅在信息结构上体现子代理特性.
...
8. 切换 API Profile 或模型只允许执行服务端配置切换,不得向 AI 注入任何额外用户消息(例如 `/profile xxx`).
...
2. 会话 Tab/列表项 红点:
   ...
   - 未查看终态树状弹窗中,同一会话需展示一致的未查看状态标记.
   - 会话级红点以单个会话为粒度,在左侧最近会话列表,会话管理弹窗与未查看终态树状弹窗中保持一致展示.
   - 红点触发条件与会话内容相关:
     - 审批请求未处理.
     - 用户提问未回复.
     - 会话在客户端断开窗口内进入终态且未查看.
...
- 断开窗口定义为: 页面在线但 SSE 断开,或页面不可用,到 SSE 重连成功并完成会话状态同步为止.
- 只有 SSE 服务端侧会话在断开窗口内进入终态时,才计入"终态未查看"提醒.
- TUI 侧完成的会话不计入本网页客户端的"终态未查看"提醒.
- 终态包含成功,失败,中断等会话停止状态.

4. 红点状态清除:

- 用户打开对应会话页面后,自动清除该会话的"终态未查看"红点.
- 用户批准或拒绝审批后,清除审批相关红点.
- 用户回复提问后,清除提问相关红点.
...
4. Tips 交互行为:
   ...
   - 未发生用户主动点击前,任何 Tips 或会话终态事件都不得触发跨服务端,跨会话自动跳转.
...
1. 工具调用发生时,聊天主区出现 1 张工具调用卡片,状态为"执行中".
2. 工具返回成功时,同一张卡片更新为"成功",不新增独立工具结果卡片.
3. 工具返回失败时,同一张卡片更新为"失败",并保留输入与输出详情展开入口.
4. 详情默认折叠,展开后可查看完整输入参数和完整返回内容.
...
1. 聊天主区出现 1 张子代理工具卡片,而不是独立子代理专用面板.
2. 卡片初始状态为"执行中",默认展示输入任务摘要.
3. 如收到工作过程,卡片显示"工作过程"折叠区并实时更新.
4. 如收到最终回复,卡片显示"子代理回复"折叠区,并将卡片状态更新为"完成".

### 例 11: 子代理部分信息缺失时的展示

场景: 子代理执行期间仅有输入任务,未收到工作过程或最终回复.

预期:

1. 卡片仅展示输入任务摘要与输入详情折叠区.
2. 不显示"工作过程"区域.
3. 不显示"子代理回复"区域.
4. 整体风格与普通工具卡片一致.
...
1. 在 `project-a` 会话点击日志按钮后打开日志弹窗.
2. 日志弹窗支持右上角 `X` 关闭.
3. 弹窗仅显示 `project-a` 当前会话事件.
4. 不混入 `project-b` 事件.
...
场景: 用户的浏览器页面在线,但与 `project-a:3000` 的 SSE 连接中断 10 分钟.这期间会话 A1 成功结束,会话 A2 失败结束,会话 A3 被中断结束.随后客户端重连成功并完成状态同步.

预期:

1. A1,A2,A3 都进入"终态未查看"集合,因为它们都在断开窗口内停止.
2. `project-a:3000` 服务端 Tab 显示红点,并显示未查看终态数量.
3. 左侧会话列表中 A1,A2,A3 均显示红点.
4. 右上角 Tips 可提示"执行完成"类提醒,但不要求逐条强制弹窗.
5. 若用户没有点击 Tips 或未查看终态弹窗中的会话入口,页面保持当前会话不变,不得自动跳转到 A1,A2,A3.
6. 用户打开 A2 会话页面后,A2 红点立即清除,A1 和 A3 红点保持.
7. 若某会话是 TUI 侧完成而非 SSE 服务端侧完成,该会话不进入本网页的"终态未查看"提醒.
...
场景: 当前仅显示每个服务端最近 5 个会话,但断开期间有更多会话进入终态未查看.

预期:

1. 页面提供"未查看终态"按钮,点击后打开树状弹窗.
2. 弹窗支持右上角 `X` 关闭,关闭后不改变未查看状态.
3. 弹窗按服务端分组折叠展示所有未查看终态会话,不受最近 5 个会话限制.
4. 每个服务端分组显示数量,例如 `project-a:3000 (7)`.
5. 用户点击弹窗中的某一会话后,先关闭弹窗,再跳转对应服务端和会话页面,并清除该会话红点.
6. 当某服务端分组下全部会话被查看后,该分组数量归零并自动收起或移除.

### 例 32: 思考块实时与历史一致性

场景: 某条代理回复包含思考内容.用户先在实时流式阶段查看该条回复,随后离开并重新进入同一会话查看历史.

预期:

1. 在实时收消息阶段,该条代理回复块立即提供"思考内容"折叠入口.
2. 折叠入口默认收起,用户可手动展开查看.
3. 用户重新进入会话查看历史时,同一条回复仍提供"思考内容"折叠入口.
4. 历史回看默认仍收起,并允许用户再次手动展开.
5. 不允许出现"实时阶段无入口,历史回看才出现入口"的展示不一致.
...
- [ ] 客户端连接后自动保持各服务端下全部运行中会话连接,无需逐个打开会话.
- [ ] 客户端从断开恢复连接后,自动补连当前仍在运行的会话并继续接收实时消息.
- [ ] 文本和图片消息发送可用.
- [ ] 审批弹窗和问题弹窗可用.
- [ ] 聊天视图下 TODO 区域常驻且不可折叠,手机端固定高度约 5 条可视并支持内部滚动,且不可手动调整高度.
- [ ] 提供日志按钮,点击后打开日志弹窗,点击弹窗右上角 `X` 可关闭.
- [ ] 日志弹窗仅显示当前会话事件流.
- [ ] 新建和恢复会话默认 YOLO 开启,可手动关闭.
- [ ] 聊天区刷新或消息更新时,输入框焦点不被抢夺,输入内容与光标位置保持稳定.
- [ ] 子代理展示并入聊天主区工具卡片,不再使用独立子代理专门展示区域.
- [ ] 子代理工具卡片支持"执行中"到"完成"状态流转,并按是否有数据决定显示"工作过程"和"子代理回复"折叠区.
- [ ] 代理回复含思考内容时,实时收消息阶段与会话历史回看阶段都提供"思考内容"折叠区,且默认收起并允许手动展开.
- [ ] 同一条回复在实时展示与历史回看中的思考块可见性规则一致,不得出现"实时无思考块,历史回看才出现"的不一致.
...
- [ ] 切换 API Profile 或模型不会向 AI 发送任何额外用户消息(例如 `/profile xxx`).
- [ ] 状态栏展示 API Profile,上下文使用百分比,已用 Token / 总 Token,KV 缓存,连接状态,YOLO 状态.
- [ ] 状态栏显示当前会话工作状态,明确区分"工作中"与"已停止".
...
- [ ] 正式 Web 客户端保留"压缩"按钮,用于当前会话标准压缩操作.
- [ ] 工具调用与工具结果在聊天主区采用单卡片状态流转: 调用时显示执行中,返回后更新同卡片为成功或失败,不额外新增独立结果卡片.
- [ ] 工具卡片支持折叠详情,可查看完整输入参数与完整返回内容,长内容默认摘要展示.
- [ ] 断开窗口内由 SSE 服务端侧进入终态的会话,在重连后会进入"终态未查看"提醒,终态包含成功,失败,中断.
- [ ] TUI 侧完成的会话不会计入本网页客户端的"终态未查看"提醒.
- [ ] 服务端 Tab,会话列表,会话管理弹窗与未查看终态树状弹窗对"终态未查看"红点展示一致.
- [ ] 页面提供"未查看终态"按钮,点击后打开树状弹窗,弹窗支持 `X` 关闭.
- [ ] 树状弹窗按服务端分组折叠展示所有未查看终态会话,不受最近 5 个会话限制.
- [ ] 用户点击树状弹窗中的会话后,弹窗关闭并跳转到对应会话,该会话"终态未查看"红点立即清除.
- [ ] 未发生用户主动点击前,会话终态到达只允许更新红点和 Tips,不得触发跨服务端或跨会话自动跳转.
- [ ] 未知 SSE 事件可容错,页面不崩溃.
- [ ] 关键聊天与交互场景通过浏览器端实际操作验证,结果与本需求文档口径一致.
