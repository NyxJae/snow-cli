  ...
  data: {
    agents: [
      ...
      },
      {
        ...
        description: 'Analyzes and clarifies user requirements'
      },
      {
        id: 'debugger',
        name: 'Debugger',
        description: 'Specializes in debugging and troubleshooting'
      }
    ],
    currentAgentId: 'general'  // 当前会话使用的主代理ID
  },
  timestamp: '2025-12-30T15:30:00.000Z'
}
```
...
- 建立 SSE 连接后会立即收到此事件
- `agents` 列表包含所有可用主代理（内置 + 用户自定义）
- `currentAgentId` 表示当前会话正在使用的主代理，默认为 `'general'`
- 用户自定义主代理配置在 `~/.snow/main-agents.toml`
...
- 发送 `switch_agent` 消息并验证通过后触发
- 只影响后续新对话，进行中的对话继续使用旧配置
- 切换后主代理的响应会体现在后续 `message` 事件中

## 主代理选择

SSE 模式支持会话级别的主代理选择，每个 SSE 会话可以独立选择和切换主代理，不同会话之间互不干扰。

### 获取可用主代理列表

建立 SSE 连接后，服务器会自动推送 `agent_list` 事件：
...
eventSource.onmessage = event => {
	...
	if (data.type === 'agent_list') {
		console.log('可用主代理:', data.data.agents);
		console.log('当前主代理:', data.data.currentAgentId);

		// 显示主代理选择界面
		data.data.agents.forEach(agent => {
			console.log(`${agent.id}: ${agent.name} - ${agent.description}`);
		});
	}
};
```
...
发送 `switch_agent` 消息切换当前会话的主代理：

```javascript
async function switchAgent(agentId, sessionId) {
	const response = await fetch('http://localhost:3000/message', {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
		...
		body: JSON.stringify({
			type: 'switch_agent',
			agentId: agentId, // 目标主代理ID，如 'debugger'
			sessionId: sessionId, // 可选，默认为当前连接的会话
		}),
	});
...
// 使用示例：切换到调试代理
await switchAgent('debugger', 'abc-123-def-456');
```
...
| 参数      | 类型   | 必填 | 说明                            |
| --------- | ------ | ---- | ------------------------------- |
| type      | string | 是   | 固定为 `'switch_agent'`         |
| agentId   | string | 是   | 目标主代理 ID，如 `'debugger'`  |
| sessionId | string | 否   | 目标会话 ID，默认为当前连接绑定 |

**响应事件**：

- 切换成功：收到 `agent_switched` 事件
- 切换失败：收到 `error` 事件，包含具体错误码
...
// 等待中断完成后再切换
await switchAgent('debugger', sessionId);
```
...
**处理建议**：使用响应中返回的 `availableAgents` 列表重新选择。
...
class SnowAIClient {
	...
	handleEvent(event) {
		switch (event.type) {
			case 'agent_list':
				// 保存可用主代理列表
				this.availableAgents = event.data.agents;
				this.currentAgentId = event.data.currentAgentId;
				console.log('可用主代理:', this.availableAgents);
				console.log('当前主代理:', this.currentAgentId);
				break;
			...
			case 'agent_switched':
				...
				this.currentAgentId = event.data.currentAgentId;
				console.log(
					`主代理已切换: ${event.data.previousAgentId} -> ${event.data.currentAgentId}`,
				);
				break;

			case 'error':
				if (event.data.errorCode === 'agent_not_found') {
					console.error('主代理不存在，可用代理:', event.data.availableAgents);
				} else if (event.data.errorCode === 'agent_busy') {
					console.error('会话正忙，请先中断当前任务');
				}
				break;
...
// 等待 agent_list 事件后，切换到调试代理
setTimeout(async () => {
	await client.switchAgent('debugger');
}, 1000);
```

### 兼容性说明

- **向后兼容**：老客户端可以忽略未知的 `agent_list` 和 `agent_switched` 事件
- **默认行为**：未实现主代理选择的客户端继续使用默认 `'general'` 代理
- **事件可选**：`agent_list` 事件在连接后自动发送，客户端可选择性处理
